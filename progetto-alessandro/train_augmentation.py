...
parser.add_argument('--arch', type=str, default = 'DECO_medium_conv',  help='model architecture: DECO, DECO_medium_conv, DECO_heavy_conv')
...
parser.add_argument('--solver', type=str, default = 'Adam',  help='model solver: Adam, SGD or Nesterov. For nesterov is suggested: train_augmentation.py --nepoch 120 --solver Nesterov --lr 0.01  --lr_gamma 0.1')
parser.add_argument('--lr', type=float, default=0.001, help='starting learning rate.')
parser.add_argument('--lr_gamma', type=float, default=1.0, help='learning rate decrease factor.Suggested 1.0 for Adam, 0.1 per SGD/Nesterov')
...
split = opt.dset
folder = '/'+split+'/'+freezed+'/fc'+str(opt.nfc)+'/'

root_weights = './weights'+folder
root_data = './data_acc_loss'+folder
...
acc_0 = open('%s/acc_%s_%d_%s.txt' %(root_data,opt.arch,opt.nfc,freezed), 'w+')
loss_f_0 = open('%s/loss_%s_%d_%s.txt' %(root_data,opt.arch,opt.nfc,freezed), 'w+')
acc_test_0  = open('%sacc_test_%s_%d_%s.txt' %(root_data,opt.arch,opt.nfc,freezed), 'w+')
loss_f_test_0 = open('%sloss_test_%s_%d_%s.txt' %(root_data,opt.arch,opt.nfc,freezed), 'w+')

labels_p = open('./numpy/labels_image_%s_%s.txt' %(freezed,split), 'w+')
...
print ("saving to " + savename + " file_%s_%s" %(opt.arch,freezed))
...
def train(epoch,lr):
    ...
    if epoch < num_epoch-10:
            if  epoch % 10 == 0:
               filesave = root_weights+'epoch'+str(epoch)+opt.arch+opt.solver+freezed+'.pkl'
...
#saving data
npArray_freezed = open('./numpy/pcl_np_%s_%s' %(freezed,split), 'w+')
